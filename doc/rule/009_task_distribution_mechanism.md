...
为支持大任务的质量控制，推荐在“命令投递到执行Agent之前”增加一个**DAG评审关口**：
...
推荐做法：在Plan开始阶段一次性投递所有 `cmd_*.json`，并为依赖任务设置 `wait_for_inputs=true` 与 `required_inputs`；后续由输入文件到达触发执行，无需GM持续调度。

若启用DAG评审关口：建议先仅投递“评审命令”给评审角色，待通过后再投递执行命令，避免错误DAG触发大量无效执行。
